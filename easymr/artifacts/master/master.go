...
import (
	...
	"github.com/xp/shorttext-db/easymr/artifacts/worker"
...
var logger = glogger.MustGetLogger("master")
...
func (m *Master) DoneMulti(ts map[int]*task.Task) error {
	...
	for _, f := range futures {
		...
		case <-f.IsDone():
			f.Close()
			continue
		case <-time.After(constants.DEFAULT_TASK_EXPIRY_TIME): // constants.DEFAULT_TASK_EXPIRY_TIME
			logger.Errorf("任务执行超时，超时设置:%d\n", constants.DEFAULT_TASK_EXPIRY_TIME)
			//f.Close()
