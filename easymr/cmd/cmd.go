...
func InitCmdEnv() *SysVars {
	...
	principal := flag.Bool("principal", constants.DEFAULT_PRINCIPAL, "define the principal server")
	...
	Init()
	return &SysVars{*debug, *port, *casePath, *logPath, *cleanHis, *numWorkers, *numGossip, *caseID, *principal}
}
...
func Combine(vars *SysVars) *SysVars {
	...
	//后面的配置覆盖前面的配置项
	vars.LogPath = cmdVar.LogPath
	vars.CasePath = cmdVar.CasePath
	vars.DebugMode = cmdVar.DebugMode
	vars.Principal = cmdVar.Principal
	...
	//}
	//	fmt.Println("222222---------------------------------------------",cmdVar.Principal)
	singleton = vars
	return vars
}
