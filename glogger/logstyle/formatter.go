...
var formatRegexp = regexp.MustCompile(`%{(color|id|level|message|module|shortfunc|time|shortfile)(?::(.*?))?}`)
...
func newTimeFormatter(f string) TimeFormatter {
	return TimeFormatter{Layout: stringOrDefault(f, "2006-01-02 15:03:05")}
}
...
func (m ShortFileFormatter) Format(w io.Writer, entry zapcore.Entry, fields []zapcore.Field) {
	fmt.Fprintf(w, m.FormatVerb+" "+"line %d", filepath.Base(entry.Caller.File), entry.Caller.Line)
}
