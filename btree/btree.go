...
// index.
//二分查找，先找到中间元素，如果大于，往后进行索引；如果小于，进行再次二分
...
//i 为当前节点Key集合的中间元素索引，例如长度为３集合，i为１
//以i 为边界分割成两部分,小于i的部分保留给当前节点，大于i 的部分赋值给新节点
//如果当前节点有子节点，子节点也按照Key的分割方式分割
...
func PrintBTree(tree *BTree) {
	root := tree.root
	name := itemsToString(root.items)
	textNode := NewTextNode(name, nil, []*TextNode{})
	for i := 0; i < len(root.children); i++ {
		createTextNode(root.children[i], textNode)
	}
	fmt.Println(textNode.PrintAsTree())
}

func itemsToString(list items) string {
	buffer := make([]string, 0)
	for i := 0; i < len(list); i++ {
		ele := list[i].(Int)
		buffer = append(buffer, strconv.Itoa(int(ele)))
	}
	return strings.Join(buffer, "|")
}

func createTextNode(dataNode *node, textNode *TextNode) {
	if dataNode == nil || len(dataNode.items) == 0 {
		return
	}
	n := NewTextNode(itemsToString(dataNode.items), textNode, []*TextNode{})
	textNode.children = append(textNode.children, n)
	for i := 0; i < len(dataNode.children); i++ {
		createTextNode(dataNode.children[i], n)
	}
}
